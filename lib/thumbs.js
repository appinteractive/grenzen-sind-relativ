const resize = async (input, output, originalSize, outputSize, blur) => {
  ...
  const doIt = !fs.existsSync(outputName) || fs.statSync(input).mtime > fs.statSync(outputName).mtime
  if (!doIt) {
...
const getThumbs = (url) => {
  ...
  return {
    url: `/thumbs/${hash}.${ext}`,
    srcset: [
      `/thumbs/${hash}.${ext} 460w`,
      `/thumbs/${hash}.${ext} 700w`,
      `/thumbs/${hash}.${ext} 1024w`,
      `/thumbs/${hash}.${ext} 2048w`,
    ]
  }
}
